...
public:
	...
	void Extend(unsigned int x, unsigned int y[2]);
	int filecryption(char *fname, char ch[9], int crypflag);
	void inttoch(_int64 temp, char ch[4]);
	void IP_PM(unsigned int x[2]);
	void IP_PM1(unsigned int x[2]);
	unsigned int LTr(unsigned int x, unsigned int y);
	void PC1(unsigned x[2]);
	void PC2(unsigned x[2]);
	unsigned int PM(unsigned int x);
	unsigned int Sbox(unsigned int x[2]);
	char IP_PERMUTE[64] = { \
