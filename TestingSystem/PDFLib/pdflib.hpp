...
// $Id: pdflib.hpp,v 1.80.2.4 2010/09/03 12:48:23 stm Exp $
//
...
public:
    ...
    static void convert_to_pdf_bytes(
        const basic_PDFlib<pstring, NoOpConverter<pstring> >& p,
...
#include "cpp_wrapped.h"
            ...
            int reserved, int *len = NULL)
    {
        ...
        utf8_to_pstring(retval, pstring_retval);
...
private:
    ...
    void check_api(void *opaque)
    {
            ...
            m_PDFlib_api->minor != PDFLIB_MINORVERSION)
        {
            ...
            utf8_to_pstring("loaded wrong version of PDFlib library", message);
            utf8_to_pstring(__FILE__, filename);

            throw Exception(message, -1, filename, opaque);
        }
    }
    ...
    void utf8_to_pstring(const char * const pdflib_retval,
